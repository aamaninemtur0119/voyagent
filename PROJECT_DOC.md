...
- **"Multi-agent" wasn't fully true until it was checked.** Asked directly "where is multi-agent here?", inspecting the code showed `logistics.py` and `experience.py` had zero LLM calls — they fetched from Google Places and did plain Python `sorted()`/filtering. Only the Eligibility Agent and the orchestrator's synthesis step actually reasoned. Fixed by giving both agents a genuine LLM curation step — picking and justifying the best options against the traveler's stated preferences, grounded strictly in the real returned fields (never inventing a cuisine, view, or amenity the API didn't return) — then re-verified both the happy path and the simulated-failure path still held after the change. Worth stating plainly: it's easy to build a system that's structurally multi-agent (real state, real delegation) while only one node actually thinks.
