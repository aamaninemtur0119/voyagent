...
class Itinerary(BaseModel):
    briefing: str = Field(
        description=(
            ...
            "plain sentence whether a price was found or not, and if so that it's sandbox/test "
            "data rather than a live market fare (e.g. 'A flight search wasn't available this "
            "time, but search links are below' or 'A sample flight option was found below (test "
            "data, not a live fare)') — do not describe HOW or WHERE it's shown beyond that. If a "
            "section is null/"
